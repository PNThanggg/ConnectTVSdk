...
import com.connect.DefaultPlatform;
...
import com.connect.device.ConnectableDeviceStore;
import com.connect.device.DefaultConnectableDeviceStore;
import com.connect.service.DLNAService;
...
public class DiscoveryManager implements ConnectableDeviceListener, DiscoveryProviderListener, ServiceConfig.ServiceConfigListener {
    ...
    public static synchronized DiscoveryManager getInstance() {
        if (instance == null) throw new Error("Call DiscoveryManager.init(Context) first");
    ...
    public void registerDeviceService(Class<? extends DeviceService> deviceClass, Class<? extends DiscoveryProvider> discoveryClass) {
        if (!DeviceService.class.isAssignableFrom(deviceClass)) return;

        if (!DiscoveryProvider.class.isAssignableFrom(discoveryClass)) return;
    ...
    public void unregisterDeviceService(Class<?> deviceClass, Class<?> discoveryClass) {
        ...
        try {
            ...
            if (discoveryProvider == null) return;
    ...
    public ConnectableDevice getDeviceById(String deviceId) {
        if (deviceId != null) {
            for (ConnectableDevice dvc : allDevices.values()) {
                if (deviceId.equals(dvc.getId())) return dvc;
            }
        }
    ...
    public ConnectableDevice getDeviceByIpAddress(String ipAddress) {
        if (ipAddress != null) {
            for (ConnectableDevice dvc : allDevices.values()) {
                if (ipAddress.equals(dvc.getIpAddress())) return dvc;
            }
        }
...
//        device.setLastSeenOnWifi(currentSSID);
    ...
    public void addServiceDescriptionToDevice(ServiceDescription desc, ConnectableDevice device) {
        ...
        if (deviceServiceClass == DLNAService.class) {
            if (desc.getLocationXML() == null) return;
        ...
        if (connectableDeviceStore != null) serviceConfig = connectableDeviceStore.getServiceConfig(desc);

        if (serviceConfig == null) serviceConfig = new ServiceConfig(desc);
