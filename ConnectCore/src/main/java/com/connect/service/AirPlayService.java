...
import com.connect.service.airplay.AirPlayServiceSocketClient;
import com.connect.service.airplay.PListParser;
...
import com.connect.service.config.AirPlayServiceConfig;
...
public class AirPlayService extends DeviceService implements MediaPlayer, MediaControl {
    ...
    private void getPlaybackPosition(final PlaybackPositionListener listener) {
        ResponseListener<Object> responseListener = new ResponseListener<Object>() {
            ...
            @Override
            public void onError(ServiceCommandError error) {
                if (listener != null) listener.onGetPlaybackPositionFailed(error);
            }
        };
    ...
    public void sendCommand(final ServiceCommand<?> serviceCommand) {
        if (socketClient != null) socketClient.sendCommand(serviceCommand);
    }
    ...
    public boolean isConnected() {
        if (DiscoveryManager.getInstance().getPairingLevel().compareTo(DiscoveryManager.PairingLevel.PROTECTED) >= 0) {
            return this.socketClient != null && this.socketClient.isConnected() && !Objects.equals(this.socketClient.getAuthToken(), "");
        } else {
            return this.socketClient != null && this.socketClient.isConnected();
        }
    }

    private AirPlayServiceSocketClient.AirPlayServiceSocketClientListener mSocketListener = new AirPlayServiceSocketClient.AirPlayServiceSocketClientListener() {
        ...
        public void onRegistrationFailed(final ServiceCommandError error) {
            ...
            Util.runOnUI(new Runnable() {
                ...
                @Override
                public void run() {
                    if (listener != null) listener.onConnectionFailure(AirPlayService.this, error);
                }
            });
        }
        ...
        public void onCloseWithError(final ServiceCommandError error) {
            ...
            Util.runOnUI(() -> {
                if (listener != null) listener.onDisconnect(AirPlayService.this, error);
            });
        }
        ...
        public void onBeforeRegister(final PairingType pairingType) {
            if (DiscoveryManager.getInstance().getPairingLevel().compareTo(DiscoveryManager.PairingLevel.ON) >= 0) {
                Util.runOnUI(() -> {
                    if (listener != null) listener.onPairingRequired(AirPlayService.this, pairingType, null);
                });
            }
        }
    };
    ...
    public void connect() {
        ...
        socketClient = new AirPlayServiceSocketClient(this.getAirPlayServiceConfig(), this.getPairingType(), this.getServiceDescription().getIpAddress());
    ...
    public void disconnect() {
        ...
        Util.runOnUI(new Runnable() {
            @Override
            public void run() {
                if (listener != null) listener.onDisconnect(AirPlayService.this, null);
            }
        });
