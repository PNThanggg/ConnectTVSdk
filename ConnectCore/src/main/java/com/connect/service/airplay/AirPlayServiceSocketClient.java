...
import com.connect.service.airplay.auth.AirPlayAuth;
import com.connect.service.airplay.auth.AuthUtils;
    ...
    public void connect() {
        ...
        try {
            ...
            try {
                ...
                if (mListener != null) mListener.onBeforeRegister(mPairingType);
            } catch (Exception ex) {
                ex.printStackTrace();
                mListener.onRegistrationFailed(new ServiceCommandError(ex.toString()));
            }
            ...
            public void run() {
                ...
                try {
                    if (serviceCommand.getHttpMethod().equalsIgnoreCase(ServiceCommand.TYPE_GET)) {
                        ...
                        Util.postSuccess(serviceCommand.getResponseListener(), AuthUtils.postData(socket, url, HttpMessage.CONTENT_TYPE_APPLICATION_PLIST, payload));
