...
public class WebOSTVServiceSocketClient extends WebSocketClient implements ServiceCommand.ServiceCommandProcessor {
    ...
    public enum State {
        NONE, INITIAL, CONNECTING, REGISTERING, REGISTERED, DISCONNECTING
    }
    ...
    public static URI getURI(WebOSTVService service) {
        String uriString = "wss://" + service.getServiceDescription().getIpAddress() + ":" + service.getServiceDescription().getPort();
        URI uri = null;
    ...
    public void disconnectWithError(ServiceCommandError error) {
        ...
        if (mListener != null) mListener.onCloseWithError(error);
    }
...
//            manifest.put("localizedAppNames", 1);
    ...
    protected void handleConnectError(Exception ex) {
        ...
        if (mListener != null) mListener.onFailWithError(new ServiceCommandError(0, "connection error", null));
    }
    ...
    protected void handleMessage(JSONObject message) {
        ...
        if (mListener != null) shouldProcess = mListener.onReceiveMessage(message);
...
//                Log.d(Util.T, "Found requests need to handle response");
                ...
                if (!(request instanceof URLServiceSubscription)) {
                    if (!(payload instanceof JSONObject && ((JSONObject) payload).has("pairingType"))) synchronized (requests) {
                        requests.remove(id);
                    }
                }
            } else {
            ...
            if (payload instanceof JSONObject) {
                ...
                if (verification_status) {
                    ...
                    if (id != null) synchronized (requests) {
                        requests.remove(id);
                    }
                } else {
            ...
            if (message.has("id")) {
                ...
                if (request != null) {
                    ...
                    if (!(request instanceof URLServiceSubscription)) synchronized (requests) {
                        requests.remove(id);
                    }
    ...
    protected void sendVerification() {
        ResponseListener<Object> listener = new ResponseListener<Object>() {
            ...
            public void onError(ServiceCommandError error) {
                ...
                if (mListener != null) mListener.onRegistrationFailed(error);
            }
    ...
    protected void sendRegister() {
        ResponseListener<Object> listener = new ResponseListener<Object>() {
            ...
            public void onSuccess(Object object) {
                if (object instanceof JSONObject) {
                    ...
                    if (mListener != null) mListener.onBeforeRegister(pairingType);
                }
            }
        };
        ...
        try {
            ...
            if (DeviceService.PairingType.PIN_CODE.equals(mPairingType)) {
    ...
    public void sendPairingKey(String pairingKey) {
        ResponseListener<Object> listener = new ResponseListener<Object>() {
            ...
            public void onError(ServiceCommandError error) {
                ...
                if (mListener != null) mListener.onFailWithError(error);
            }
...
//        connectableDeviceStore.addDevice(storedDevice);
    ...
    protected void setupSSL() {
        ...
        } catch (KeyException | NoSuchAlgorithmException ignored) {
        }
    }
    ...
    private void handleConnectionLost(boolean cleanDisconnect, Exception ex) {
        ...
        if (ex != null || !cleanDisconnect) error = new ServiceCommandError(0, "conneciton error", ex);
