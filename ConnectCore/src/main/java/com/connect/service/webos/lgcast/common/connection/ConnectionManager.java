...
import com.connect.device.ConnectableDeviceListener;
...
public class ConnectionManager implements ConnectableDeviceListener {
    ...
    public void openConnection(ConnectableDevice connectableDevice, ConnectionManagerListener connectionListener) {
        ...
        try {
            ...
            if (mConnectableDevice == null || mConnectionManagerListener == null || mLGCastCommand == null)
    ...
    public void closeConnection() {
        ...
        if (mConnectableDevice != null && !mKeepConnection) mConnectableDevice.disconnect();
    ...
    public void notifyScreenOnOff(boolean isOn) {
        ...
        if (mCurrentState == ConnectionState.CONNECTED) {
            mConnectionHandler.post(() -> {
                ...
                if (isOn) mLGCastCommand.sendPlay();
    ...
    public void setSourceDeviceCapability(JSONObject capability, JSONObject mobileDescription) {
        ...
        if (mLGCastCommand == null) return;
        if (mCurrentState == ConnectionState.CONNECTED)
            mConnectionHandler.post(() -> mLGCastCommand.sendSetParameter(mServiceName, capability, mobileDescription));
    ...
    public void updateSourceDeviceCapability(JSONObject capability) {
        ...
        if (mLGCastCommand == null) return;
        if (mCurrentState == ConnectionState.CONNECTED)
            mConnectionHandler.post(() -> mLGCastCommand.sendSetParameter(mServiceName, capability, null));
    ...
    public void sendGetParameterResponse(JSONObject parameter) {
        ...
        if (mLGCastCommand == null) return;
        if (mCurrentState == ConnectionState.CONNECTED && parameter != null)
            mConnectionHandler.post(() -> mLGCastCommand.sendGetParameterResponse(mServiceName, parameter));
        else callOnConnectionFailed("Device NOT connected");
    }
    ...
    public void sendSetParameterResponse(JSONObjectEx parameter) {
        ...
        if (mLGCastCommand == null) return;
        if (mCurrentState == ConnectionState.CONNECTED && parameter != null)
            mConnectionHandler.post(() -> mLGCastCommand.sendSetParameterResponse(mServiceName, parameter.toJSONObject()));
        else callOnConnectionFailed("Device NOT connected");
    }
    ...
    public void onDeviceDisconnected(ConnectableDevice device) {
        ...
        else if (mCurrentState == ConnectionState.CONNECTED)
    ...
    private void subscribe() {
        ...
        if (mLGCastCommand != null) mLGCastCommand.subscribeForServiceCommand(response -> {
            ...
            if (response.has(KEY_SUBSCRIBED)) handleSubscribed(response);
            else if (response.has(KEY_CMD)) handleCommand(response);
        ...
        if (mLGCastCommand != null) mLGCastCommand.subscribeForPowerState(response -> {
            ...
            if (VAL_REQUEST_POWER_OFF.equalsIgnoreCase(processing))
    ...
    private void handleSubscribed(@NonNull JSONObject response) {
        ...
        if (response.optBoolean(KEY_SUBSCRIBED, false)) sendConnect();
    ...
    private void sendConnect() {
        ...
        if (mLGCastCommand == null) return;
        if (mLGCastCommand.sendConnect(mServiceName)) getParameter();
    ...
    private void handleCommand(@NonNull JSONObject jsonObj) {
        ...
        if (!targetClientKey.equals(mLGCastCommand.getClientKey())) {
