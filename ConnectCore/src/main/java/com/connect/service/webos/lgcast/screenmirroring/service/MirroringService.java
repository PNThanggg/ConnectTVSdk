package com.connect.service.webos.lgcast.screenmirroring.service;
...
import com.connect.service.webos.lgcast.screenmirroring.ScreenMirroringConst;
import com.connect.service.webos.lgcast.screenmirroring.capability.MirroringSinkCapability;
import com.connect.service.webos.lgcast.screenmirroring.capability.MirroringSourceCapability;
import com.connect.service.webos.lgcast.screenmirroring.capability.VideoSizeInfo;
...
public class MirroringService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mServiceHandler.post(() -> {
            if (MirroringServiceIF.ACTION_START_REQUEST.equals(action)) executeStart(intent);
            else if (MirroringServiceIF.ACTION_STOP_REQUEST.equals(action)) executeStop();
            else if (MirroringServiceIF.ACTION_STOP_BY_NOTIFICATION.equals(action)) executeStopByNotification();
        });
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mConnectionManager == null || mMirroringSinkCapability == null || mMirroringSourceCapability == null)
    ...
    private void executeStart(Intent intent) {
        ConnectionManagerListener connectionListener = new ConnectionManagerListener() {
            ...
            public void onConnectionCompleted(@NonNull JSONObject jsonObj) {
                ...
                UibcAccessibilityService.onDisplayRotated(mMirroringSinkCapability.displayOrientation);
            }
            ...
            public void onReceiveSetParameter(@NonNull JSONObject jsonObj) {
                ...
                if (mMirroringSinkCapability != null && mMirroringSinkCapability.isSupportPortraitMode()) {
    ...
    private void openTvConnection(Intent intent, ConnectionManagerListener connectionListener) {
        ...
        if (ScreenMirroringConfig.Test.usePcPlayer) {
    ...
    private boolean startCaptureAndStreaming(Intent intent) {
        ...
        try {
            ...
            if (ScreenMirroringConfig.Test.testMkiUpdate) {
                TimerUtil.schedule(() -> {
            ...
            if (ScreenMirroringConfig.Test.testOrientationChange) {
                TimerUtil.schedule(() -> {
