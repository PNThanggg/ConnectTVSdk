...
public class WebOSTVDeviceService extends DeviceService implements PowerControl, MediaControl {
    ...
    public interface WebOSTVServicePermission {
        enum Open implements WebOSTVServicePermission {
            LAUNCH, LAUNCH_WEB, APP_TO_APP, CONTROL_AUDIO, CONTROL_INPUT_MEDIA_PLAYBACK
        }

        WebOSTVServicePermission[] OPEN = {Open.LAUNCH, Open.LAUNCH_WEB, Open.APP_TO_APP, Open.CONTROL_AUDIO, Open.CONTROL_INPUT_MEDIA_PLAYBACK};

        enum Protected implements WebOSTVServicePermission {
            CONTROL_POWER, READ_INSTALLED_APPS, CONTROL_DISPLAY, CONTROL_INPUT_JOYSTICK, CONTROL_INPUT_MEDIA_RECORDING, CONTROL_INPUT_TV, READ_INPUT_DEVICE_LIST, READ_NETWORK_STATE, READ_TV_CHANNEL_LIST, WRITE_NOTIFICATION_TOAST, CONTROL_BLUETOOTH, CHECK_BLUETOOTH_DEVICE, CONTROL_TV_SCREEN
        }

        WebOSTVServicePermission[] PROTECTED = {Protected.CONTROL_POWER, Protected.READ_INSTALLED_APPS, Protected.CONTROL_DISPLAY, Protected.CONTROL_INPUT_JOYSTICK, Protected.CONTROL_INPUT_MEDIA_RECORDING, Protected.CONTROL_INPUT_TV, Protected.READ_INPUT_DEVICE_LIST, Protected.READ_NETWORK_STATE, Protected.READ_TV_CHANNEL_LIST, Protected.WRITE_NOTIFICATION_TOAST, Protected.CONTROL_BLUETOOTH, Protected.CHECK_BLUETOOTH_DEVICE, Protected.CONTROL_TV_SCREEN};

        enum PersonalActivity implements WebOSTVServicePermission {
    ...
    public final static String[] kWebOSTVServiceOpenPermissions = {"LAUNCH", "LAUNCH_WEBAPP", "APP_TO_APP", "CONTROL_AUDIO", "CONTROL_INPUT_MEDIA_PLAYBACK", "UPDATE_FROM_REMOTE_APP"};

    public final static String[] kWebOSTVServiceProtectedPermissions = {"CONTROL_POWER", "READ_INSTALLED_APPS", "CONTROL_DISPLAY", "CONTROL_INPUT_JOYSTICK", "CONTROL_INPUT_MEDIA_RECORDING", "CONTROL_INPUT_TV", "READ_INPUT_DEVICE_LIST", "READ_NETWORK_STATE", "READ_TV_CHANNEL_LIST", "WRITE_NOTIFICATION_TOAST", "CONTROL_BLUETOOTH", "CHECK_BLUETOOTH_DEVICE", "CONTROL_USER_INFO", "CONTROL_TIMER_INFO", "READ_SETTINGS", "CONTROL_TV_SCREEN"};

    public final static String[] kWebOSTVServicePersonalActivityPermissions = {"CONTROL_INPUT_TEXT", "CONTROL_MOUSE_AND_KEYBOARD", "READ_CURRENT_CHANNEL", "READ_RUNNING_APPS"};
    ...
    protected DeviceService getDLNAService() {
        ...
        if (allDevices != null && !allDevices.isEmpty()) device = allDevices.get(this.serviceDescription.getIpAddress());

        if (device != null) service = device.getServiceByName("DLNA");
    ...
    protected ServiceCommand<ResponseListener<Object>> getMuteStatus(boolean isSubscription, final VolumeControl.MuteListener listener) {
        ...
        else request = new ServiceCommand<>(this, MUTE, null, true, responseListener);
    ...
    protected ServiceCommand<ResponseListener<Object>> getVolumeStatus(boolean isSubscription, final VolumeControl.VolumeStatusListener listener) {
        ...
        else request = new ServiceCommand<ResponseListener<Object>>(this, VOLUME_STATUS, null, true, responseListener);
    ...
    protected ChannelInfo parseRawChannelData(JSONObject channelRawData) {
        ...
        try {
            if (!channelRawData.isNull("channelName")) channelName = (String) channelRawData.get("channelName");

            if (!channelRawData.isNull("channelId")) channelId = (String) channelRawData.get("channelId");

            channelNumber = channelRawData.optString("channelNumber");

            if (!channelRawData.isNull("majorNumber")) majorNumber = (Integer) channelRawData.get("majorNumber");
            else majorNumber = parseMajorNumber(channelNumber);

            if (!channelRawData.isNull("minorNumber")) minorNumber = (Integer) channelRawData.get("minorNumber");
            else minorNumber = parseMinorNumber(channelNumber);
    ...
    protected ServiceCommand<ResponseListener<Object>> getCurrentChannel(boolean isSubscription, final TVControl.ChannelListener listener) {
        ...
        } else request = new ServiceCommand<ResponseListener<Object>>(this, CHANNEL, null, true, responseListener);
    ...
    protected ServiceCommand<ResponseListener<Object>> getChannelList(boolean isSubscription, final TVControl.ChannelListListener listener) {
        ...
        else request = new ServiceCommand<>(this, CHANNEL_LIST, null, true, responseListener);
    ...
    protected ServiceCommand<ResponseListener<Object>> getChannelCurrentProgramInfo(boolean isSubscription, final TVControl.ProgramInfoListener listener) {
        ...
        else request = new ServiceCommand<>(this, uri, null, true, responseListener);
    ...
    protected ServiceCommand<ResponseListener<Object>> getProgramList(boolean isSubscription, final TVControl.ProgramListListener listener) {
        ...
        else request = new ServiceCommand<ResponseListener<Object>>(this, PROGRAM, null, true, responseListener);
    ...
    protected ServiceCommand<Launcher.AppInfoListener> getRunningApp(boolean isSubscription, final Launcher.AppInfoListener listener) {
        ...
        else request = new ServiceCommand<>(this, FOREGROUND_APP, null, true, responseListener);
...
//            params.put("appId", launchSession.getAppId());
    ...
    protected ServiceCommand<VolumeControl.VolumeListener> getVolume(boolean isSubscription, final VolumeControl.VolumeListener listener) {
        ...
        else request = new ServiceCommand<>(this, VOLUME, null, true, responseListener);
