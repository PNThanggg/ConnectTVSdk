...
public class StringUtil {
    ...
    public static String toString(Intent intent) {
        ...
        if (action != null) buffer.append("Action: ").append(action).append("\n");

        if (bundle != null) for (String key : bundle.keySet())
            buffer.append("Data: ").append(key).append("=").append(bundle.get(key)).append("\n");
    ...
    public static String toString(List<?> list, String delim) {
        ...
        if (list != null) for (Object obj : list)
            buffer.append((buffer.length() > 0) ? delim : "").append(obj.toString());
    ...
    public static String toString(Object[] array, String delim) {
        ...
        if (array != null) for (Object obj : array)
            buffer.append(obj.toString()).append(delim);
    ...
    public static String toHexString(byte[] bytes, String delim) {
        ...
        if (bytes != null) for (byte b : bytes)
            buffer.append((buffer.length() > 0) ? delim : "").append(String.format("%02x", b & 0xff));
    ...
    public static ArrayList<String> split(String message, String delim, int limit) {
        if (message == null || delim == null) return new ArrayList<String>();

        ArrayList<String> tokenList = new ArrayList<>();
        String[] tokens = (limit > 0) ? message.split(delim, limit) : message.split(delim);

        for (String token : tokens)
            if (!StringUtil.empty(token.trim())) tokenList.add(token.trim());
    ...
    {
        data = (!StringUtil.empty(data)) ? data.trim() : null;
        data = (!StringUtil.empty(data)) ? data : null;
        return data;
    }
    ...
    public static String appendURLParameter(String url, String name, String value) {
        return (url != null && name != null) ? Uri.parse(url).buildUpon().appendQueryParameter(name, value).toString() : url;
    }
