package com.connect.service.webos.lgcast.remotecamera.capture;
...
import com.connect.service.webos.lgcast.remotecamera.service.CameraProperty;
import com.connect.service.webos.lgcast.remotecamera.service.CameraUtility;
...
public class CameraCapture {
    ...
    private final AtomicReference<Handler> mVideoStreamHandler;

    private CameraCaptureBuilder mCameraCaptureBuilder;
    ...
    public void start(@NonNull CameraProperty cameraProperty) throws Exception {
        ...
        int jpegQuality = (isMaxResolution) ? RemoteCameraConfig.Camera.JPEG_QUALITY_70 : RemoteCameraConfig.Camera.JPEG_QUALITY_90;
    ...
    public boolean changeBrightness(int value) {
        ...
        try {
            if (!mCameraCaptureBuilder.setBrightness(value)) throw new Exception("setBrightness failed");
            mCameraCaptureSession.setRepeatingRequest(mCameraCaptureBuilder.build(), null, null);
            return true;
        } catch (Exception e) {
            Logger.error(e);
            return false;
    ...
    public boolean changeWhiteBalance(int value) {
        ...
        try {
            if (!mCameraCaptureBuilder.setWhiteBalance(value)) throw new Exception("setWhiteBalance failed");
            mCameraCaptureSession.setRepeatingRequest(mCameraCaptureBuilder.build(), null, null);
            return true;
        } catch (Exception e) {
            Logger.error(e);
    ...
    public boolean changeAutoWhiteBalance(boolean auto) {
        ...
        try {
            if (!mCameraCaptureBuilder.setAutoWhiteBalance(auto)) throw new Exception("setAutoWhiteBalance failed");
            mCameraCaptureSession.setRepeatingRequest(mCameraCaptureBuilder.build(), null, null);
            return true;
        } catch (Exception e) {
            Logger.error(e);
    ...
    private void executeCapture(@NonNull CameraProperty cameraProperty, @NonNull String cameraId, Surface previewSurface, Surface readerSurface) {
        ...
        try {
            ...
            mCameraCaptureBuilder = new CameraCaptureBuilder(mContext, mCameraDevice, cameraId);
            mCameraCaptureBuilder.addTarget(previewSurface);
            mCameraCaptureBuilder.addTarget(readerSurface);
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(previewSurface, readerSurface), new CameraCaptureSession.StateCallback() {
                ...
                public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                    ...
                    try {
                        mCameraCaptureBuilder.setBrightness(cameraProperty.brightness);
                        mCameraCaptureBuilder.setWhiteBalance(cameraProperty.whiteBalance);
                        mCameraCaptureBuilder.setAutoWhiteBalance(cameraProperty.autoWhiteBalance);
