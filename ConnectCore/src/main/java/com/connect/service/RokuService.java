...
import com.connect.service.roku.RokuApplicationListParser;
...
public class RokuService extends DeviceService implements Launcher, MediaPlayer, MediaControl, KeyControl, TextInputControl {
    ...
    private static final List<String> registeredApps = new ArrayList<>();
    ...
    public void launchAppWithInfo(final AppInfo appInfo, Object params, final Launcher.AppLaunchListener listener) {
        ...
        if (params instanceof JSONObject) {
            ...
            while (jsonIterator.hasNext()) {
                ...
                if (urlSafeKey == null || urlSafeValue == null) continue;
        ...
        if (!queryParams.isEmpty()) targetURL = baseTargetURL + queryParams;
        else targetURL = baseTargetURL;
                ...
                for (AppInfo appInfo : appList) {
                    if (appInfo.getName().equalsIgnoreCase("Netflix")) {
                        ...
                        try {
                            ...
                            if (contentId != null && !contentId.isEmpty()) payload.put("contentId", contentId);
    ...
    private void displayMedia(String url, String mimeType, String title, String description, String iconSrc, final MediaPlayer.LaunchListener listener) {
        ...
        if (mimeType.contains("image")) {
            ...
            param = String.format("15985?t=v&u=%s&k=(null)&videoName=%s&videoFormat=%s", HttpMessage.encode(url), TextUtils.isEmpty(title) ? "(null)" : HttpMessage.encode(title), HttpMessage.encode(mediaFormat));
        } else { // if (mimeType.contains("audio")) {
            param = String.format("15985?t=a&u=%s&k=(null)&songname=%s&artistname=%s&songformat=%s&albumarturl=%s", HttpMessage.encode(url), TextUtils.isEmpty(title) ? "(null)" : HttpMessage.encode(title), TextUtils.isEmpty(description) ? "(null)" : HttpMessage.encode(description), HttpMessage.encode(mediaFormat), TextUtils.isEmpty(iconSrc) ? "(null)" : HttpMessage.encode(iconSrc));
        }
    ...
    private String requestURL(String action, String parameter) {
        ...
        if (parameter != null) sb.append("/").append(parameter);
    ...
    public void disconnect() {
        ...
        Util.runOnUI(() -> {
            if (listener != null) listener.onDisconnect(RokuService.this, null);
        });
    }
